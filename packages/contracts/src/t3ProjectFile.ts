...
/** Legacy-compatible filename of the checked-in Dispatch project file. */
...
export const T3ProjectFileScript = Schema.Struct({
  name: trimmedNonEmpty({
    description: "Display name for the script, shown in the Dispatch scripts menu.",
  }),
  command: trimmedNonEmpty({
    description: "Shell command executed in a Dispatch terminal at the project root.",
  }),
  ...
  description: "A project script that team members can import into Dispatch.",
});
...
export const T3ProjectFile = Schema.Struct({
  ...
  iconPath: Schema.optionalKey(
    trimmedNonEmpty(
      {
        description:
          'Workspace-relative path to the project icon (e.g. "assets/logo.svg"). Checked before Dispatch\'s built-in icon locations.',
      },
  ...
  defaultThreadEnvMode: Schema.optionalKey(
    ThreadEnvMode.annotate({
      description:
        'Where new threads start for this repository: "worktree" for a fresh git worktree, "local" for the current checkout. A per-project setting in Dispatch overrides this; when neither is set, the global default applies.',
    }),
  ),
  scripts: Schema.optionalKey(
    Schema.Array(T3ProjectFileScript)
      .annotate({
        description: "Project scripts shared with everyone who opens this repository in Dispatch.",
      })
      .check(Schema.isMaxLength(T3_PROJECT_FILE_MAX_SCRIPTS)),
  ),
}).annotate({
  title: "Dispatch project file",
  description:
    "Checked-in project configuration for Dispatch (t3.json at the repository root). The filename and legacy schema URL are retained for compatibility.",
});
