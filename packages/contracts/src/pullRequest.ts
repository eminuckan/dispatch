...
> = {
  ...
  forgejo: {
    ...
    unauthenticated:
      "Authenticate your Forgejo or Gitea server with `fj --host <server-url> auth add-token` on the Dispatch server. If fj is missing or unconfigured for that server, use `tea login add`. A configured fj account must be repaired with fj.",
  },
  gitlab: {
    missing:
  ...
  bitbucket: {
    missing:
      "Bitbucket needs API credentials on the server. Set DISPATCH_BITBUCKET_EMAIL and DISPATCH_BITBUCKET_API_TOKEN, or DISPATCH_BITBUCKET_ACCESS_TOKEN.",
    unauthenticated:
      "Bitbucket rejected the configured credentials. Check DISPATCH_BITBUCKET_EMAIL and DISPATCH_BITBUCKET_API_TOKEN.",
  },
};
