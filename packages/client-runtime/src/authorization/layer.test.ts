...
const makeHarness = Effect.fn("TestRemoteAuthorization.makeHarness")((input: {
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        Layer.succeed(ClientCapabilities.ClientPresentation, {
          metadata: { label: "Dispatch Test", deviceType: "mobile", os: "test" },
          scopes: AuthStandardClientScopes,
        }),
      ),
    ),
  );
  return Effect.succeed({ fetch, layer });
});
