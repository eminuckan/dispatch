...
describe("searchSettings", () => {
  ...
  it("finds settings that used to be reachable only through their section", () => {
    ...
    expect(
      searchSettings("push notifications").some((item) => item.id === "publish-agent-activity"),
    ).toBe(false);
  ...
  it("keeps the local toggle searchable without account-mediated cloud controls", () => {
    const availability = {
      hasEnvironment: true,
    ...
    expect(browser).not.toContain("t3-connect");
