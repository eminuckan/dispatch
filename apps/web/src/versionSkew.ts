import type { EnvironmentId, ServerConfig, ServerSelfUpdateCapability } from "@dispatch/contracts";
import type { ServerUpdateState } from "@dispatch/client-runtime/state/server";
...
const VERSION_MISMATCH_DISMISSALS_STORAGE_KEY = "dispatch:version-mismatch-dismissals:v1";
...
/**
 * The skew a user can act on: the connected server runs an older Dispatch than
...
export function resolveVersionMismatch(
  ...
  return {
    ...
    hint: "Version mismatch. Try syncing the client and server to the same Dispatch version.",
  };
}
