...
describe("Pierre file icons", () => {
  ...
  it("extends Pierre with Dispatch-specific exact filename icons", () => {
    assert.equal(resolvePierreIconForEntry("AGENTS.md", "file")?.name, "dispatch-file-icon-agents");
    assert.equal(
      resolvePierreIconForEntry("pnpm-lock.yaml", "file")?.name,
      "dispatch-file-icon-pnpm",
    );
    assert.equal(
  ...
  it("ships every custom icon referenced by the extended resolver", () => {
    const customIconNames = new Set(
      Object.values(DISPATCH_PIERRE_ICONS.byFileName).filter((name) =>
        name.startsWith("dispatch-"),
      ),
    );
    for (const iconName of customIconNames) {
      assert.include(DISPATCH_PIERRE_ICONS.spriteSheet, `id="${iconName}"`);
    }
  });
