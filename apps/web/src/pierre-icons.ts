...
import { VIDEO_FILE_EXTENSIONS } from "@dispatch/shared/video";
...
const PIERRE_ICON_SPRITE_ID = "dispatch-pierre-file-icon-sprite";

const DISPATCH_FILE_ICON_SPRITE = `
<svg xmlns="http://www.w3.org/2000/svg" width="0" height="0" aria-hidden="true">
  <!-- Lucide Film icon, ISC license. -->
  <symbol id="dispatch-file-icon-video" viewBox="0 0 24 24">
  ...
  <symbol id="dispatch-file-icon-agents" viewBox="0 0 32 32">
  ...
  <symbol id="dispatch-file-icon-pnpm" viewBox="0 0 32 32">
...
export const DISPATCH_PIERRE_ICONS = {
  set: "complete",
  colored: true,
  spriteSheet: DISPATCH_FILE_ICON_SPRITE,
  byFileName: {
    "package.json": "file-tree-builtin-npm",
    "tsconfig.json": "file-tree-builtin-typescript",
    "agents.md": "dispatch-file-icon-agents",
    "pnpm-lock.yaml": "dispatch-file-icon-pnpm",
    "pnpm-workspace.yaml": "dispatch-file-icon-pnpm",
  },
  byFileExtension: Object.fromEntries(
    VIDEO_FILE_EXTENSIONS.map((extension) => [extension, "dispatch-file-icon-video"]),
  ),
} satisfies FileTreeIcons;

const completeIconResolver = createFileTreeIconResolver(DISPATCH_PIERRE_ICONS);
...
export function ensurePierreIconSprite(): void {
  ...
  container.innerHTML = `${getBuiltInSpriteSheet("complete")}${DISPATCH_FILE_ICON_SPRITE}`;
