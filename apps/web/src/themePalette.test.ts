...
import { BUILT_IN_THEMES } from "@dispatch/shared/themePalettes";
...
describe("theme files", () => {
  ...
  it("canonicalizes the explicitly exported theme", () => {
    const serialized = serializeThemeFile({
      ...DISPATCH_CHAT_THEME,
      colors: { ...DISPATCH_CHAT_THEME.colors, accent: "hsl(263 70% 58%)" },
    });
    expect(JSON.parse(serialized)).toMatchObject({
      version: THEME_FILE_VERSION,
      id: DISPATCH_CHAT_THEME.id,
      name: DISPATCH_CHAT_THEME.label,
      appearance: "light",
  ...
  it("suppresses sidebar artwork during a live custom-theme preview", () => {
    ...
    applyThemeColorPreview(DISPATCH_CHAT_THEME.colors, "light");
  ...
  it("keeps optional light and dark palettes under one theme id", () => {
    ...
    expect(getThemeModes(DISPATCH_CHAT_THEME)).toEqual(["light", "dark"]);
    expect(resolveThemeAppearance(DISPATCH_CHAT_THEME.id, true, true)).toBe("dark");
    expect(resolveDesktopTheme(DISPATCH_CHAT_THEME.id, true)).toBe("system");
    expect(resolveThemeAppearance(DISPATCH_CHAT_THEME.id, false, false, "dark")).toBe("dark");
    expect(resolveDesktopTheme(DISPATCH_CHAT_THEME.id, false, "dark")).toBe("dark");
  ...
  it("keeps the Dispatch Chat palette faithful and readable", () => {
    expectThemeColors(DISPATCH_CHAT_THEME.colors, {
      canvas: "#fdf7fd",
      chrome: "#fdf7fd",
    ...
    expectThemeColors(DISPATCH_CHAT_THEME.variants!.dark!, {
      canvas: "#1f1a24",
      chrome: "#1f1a24",
      surface: "#29232d",
    ...
    for (const mode of ["light", "dark"] as const) {
      const colors = getThemeColorsForMode(DISPATCH_CHAT_THEME, mode)!;
      expect(contrastRatio(colors.text, colors.canvas)).toBeGreaterThanOrEqual(7);
      expect(contrastRatio(colors.textMuted, colors.canvas)).toBeGreaterThanOrEqual(4.5);
      expect(contrastRatio(colors.messageForeground, colors.messageSurface)).toBeGreaterThanOrEqual(
        4.5,
...
describe("stored theme preferences", () => {
  ...
  it("resolves legacy Dispatch Chat preferences onto the canonical theme", () => {
    expect(getThemeDefinition("t3-chat")).toBe(DISPATCH_CHAT_THEME);
    expect(getThemeDefinition("t3-chat-dark")).toBe(DISPATCH_CHAT_THEME);
    expect(canonicalThemePreference("t3-chat")).toBe(DISPATCH_CHAT_THEME.id);
    expect(canonicalThemePreference("t3-chat-dark")).toBe(DISPATCH_CHAT_THEME.id);
  ...
  it("recognizes only preferences the runtime can render", () => {
    for (const preference of ["light", "dark", "system", DISPATCH_CHAT_THEME.id, GROVE_THEME.id]) {
...
describe("singleAppearanceOf", () => {
  it("reports the only half a theme can claim, and null for a pair", () => {
    const { variants: _pair, ...base } = DISPATCH_CHAT_THEME;
    expect(singleAppearanceOf({ ...base, id: "x", appearance: "dark" })).toBe("dark");
    expect(singleAppearanceOf(DISPATCH_CHAT_THEME)).toBe(null);
  });
});
