...
import { DEV_PROXIED_PATH_PREFIXES } from "@dispatch/shared/devProxy";
...
const isSingleOriginDev =
  (process.env.DISPATCH_SINGLE_ORIGIN_DEV ?? process.env.T3CODE_SINGLE_ORIGIN_DEV) === "1";
...
const sourcemapEnv = (process.env.DISPATCH_WEB_SOURCEMAP ?? process.env.T3CODE_WEB_SOURCEMAP)
  ?.trim()
  .toLowerCase();
...
// as hot patches. Opt-in while experimental: DISPATCH_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); DISPATCH_BUNDLED_DEV=0 opts out.
const bundledDevEnv = (process.env.DISPATCH_BUNDLED_DEV ?? process.env.T3CODE_BUNDLED_DEV)
  ?.trim()
  .toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // DISPATCH_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(
  process.env.DISPATCH_PORT ?? process.env.T3CODE_PORT,
  configuredWsUrl,
);
...
const configuredAllowedHosts = (
  process.env.DISPATCH_DEV_ALLOWED_HOSTS ??
