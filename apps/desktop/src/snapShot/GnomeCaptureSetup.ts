...
import type { DesktopCaptureExtensionState } from "@dispatch/contracts";
    ...
    try {
      ...
      if (installed && (!info.state || (info.version && installed.version > info.version.value)))
        return {
          ...
          message:
            "Installed. Save your work, sign out of GNOME and sign back in, then return here to enable the extension. Restarting Dispatch alone is not enough.",
        };
      ...
      if (!properties.UserExtensionsEnabled.value)
        return {
          ...
          message:
            "GNOME has disabled user extensions. Turn on Extensions in the GNOME Extensions app, then check again. Dispatch will not enable your other extensions for you.",
        };
