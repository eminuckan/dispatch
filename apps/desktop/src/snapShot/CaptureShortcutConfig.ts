...
export class CaptureShortcutConfig {
  ...
  async apply(id: string, desktop: Target["desktop"]): Promise<DesktopCaptureConfigApplied> {
    ...
    const temporary = NodePath.join(directory, `.dispatch-capture-${NodeCrypto.randomUUID()}.tmp`);
    let staged = false;
    try {
      ...
      await unchanged();
      const backupPath = `${root.resolvedPath}.dispatch-capture-backup-${NodeCrypto.randomUUID()}`;
