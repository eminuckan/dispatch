...
<header>↑ Drag Dispatch into the list above</header>
<button id="app" draggable="true" aria-label="Drag Dispatch to System Settings, or click to reveal in Finder"><img src="${escapeHtml(icon)}" alt="" draggable="false">Dispatch</button>
</main></body></html>`;
}
...
export class MacPermissionHelper {
  ...
  async show(
    ...
    if (!appIcon) throw new Error("The packaged Dispatch icon is missing.");
