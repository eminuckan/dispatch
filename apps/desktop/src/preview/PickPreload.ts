...
const OVERLAY_ATTRIBUTE = "data-dispatch-annotation-ui";
const Z_INDEX_OVERLAY = 2147483646;
const PRIMARY = "var(--dispatch-primary)";
const PRIMARY_FILL = "color-mix(in srgb, var(--dispatch-primary) 10%, transparent)";
...
const applyAnnotationTheme = (
  ...
  const variables = {
    "--dispatch-radius": theme.radius,
    "--dispatch-background": theme.background,
    "--dispatch-foreground": theme.foreground,
    "--dispatch-popover": theme.popover,
    "--dispatch-popover-foreground": theme.popoverForeground,
    "--dispatch-primary": theme.primary,
    "--dispatch-primary-foreground": theme.primaryForeground,
    "--dispatch-muted": theme.muted,
    "--dispatch-muted-foreground": theme.mutedForeground,
    "--dispatch-accent": theme.accent,
    "--dispatch-accent-foreground": theme.accentForeground,
    "--dispatch-border": theme.border,
    "--dispatch-input": theme.input,
    "--dispatch-ring": theme.ring,
    "--dispatch-font-sans": theme.fontSans,
    "--dispatch-font-mono": theme.fontMono,
  };
...
function startAnnotation(): void {
  ...
  cursorStyle.textContent = `html[data-dispatch-annotation-tool] body, html[data-dispatch-annotation-tool] body * { cursor: crosshair !important; } [${OVERLAY_ATTRIBUTE}], [${OVERLAY_ATTRIBUTE}] * { cursor: default !important; } [${OVERLAY_ATTRIBUTE}] input[type=number]::-webkit-inner-spin-button, [${OVERLAY_ATTRIBUTE}] input[type=number]::-webkit-outer-spin-button { appearance:none; margin:0; }`;
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-dispatch-annotation-tool", tool);
  };
      ...
      if (isUsableRect(rect)) {
        ...
        if (found === 0) {
          ...
          const regionBox = createBox(
            PRIMARY,
            "color-mix(in srgb, var(--dispatch-primary) 6%, transparent)",
          );
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-dispatch-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;
