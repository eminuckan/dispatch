...
import { HostProcessUserId } from "@dispatch/shared/hostProcess";
  ...
  const server = NodeNet.createServer((socket) => {
    ...
    socket.on("data", (chunk) => {
      ...
      void input.handle(parsed).then(finish, () => {
        finish(
          invalidResponse(parsed.requestId, "Dispatch could not process the desktop app request."),
        );
      });
    });
...
>()("@dispatch/desktop/app/DesktopAppActivation") {}
