...
import { buildRemoteNodeEnvScript } from "@dispatch/ssh/tunnel";
...
>()("@dispatch/desktop/wsl/DesktopWslEnvironment") {}
...
export const buildWslRuntimeInstallScript = (
  ...
  return [
    "set -eu",
    'runtime_parent="$HOME/.dispatch/wsl-runtime"',
...
const formatNodePtyProbeFailureReason = (exitCode: number): string | null =>
  exitCode === NODE_PTY_BINARY_MISSING_EXIT_CODE
    ? "WSL support is missing from this Dispatch build: the packaged Linux node-pty binary was not included. Install a build that includes WSL support."
    : null;
