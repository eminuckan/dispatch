...
describe("electron development launcher", () => {
  ...
  it("keeps the launcher script free of volatile environment values", () => {
    ...
    assert.include(
      script,
      "exec '/repo/node_modules/electron/Electron' --dispatch-dev-root='/repo/apps/desktop' '/repo/apps/desktop/dist-electron/main.cjs' \"$@\"",
    );
  });
  ...
  it("keeps the native Electron executable name inside the branded macOS bundle", () => {
    const paths = resolveMacLauncherPaths(
      "/repo/apps/desktop/.electron-runtime/Dispatch (Dev).app",
      "Dispatch (Dev)",
    );

    assert.equal(paths.launcherExecutableName, "Dispatch (Dev) Launcher");
    assert.equal(
      paths.launcherBinaryPath,
      "/repo/apps/desktop/.electron-runtime/Dispatch (Dev).app/Contents/MacOS/Dispatch (Dev) Launcher",
    );
    assert.equal(
      paths.runtimeElectronBinaryPath,
      "/repo/apps/desktop/.electron-runtime/Dispatch (Dev).app/Contents/MacOS/Electron",
    );
    ...
    assert.include(
      script,
      "exec '/repo/apps/desktop/.electron-runtime/Dispatch (Dev).app/Contents/MacOS/Electron'",
    );
    assert.notInclude(script, "node_modules/electron");
  });

  it("declares why the macOS app needs protected access", () => {
    const values = resolveMacBundleInfoPlistStrings("Dispatch (Dev) Launcher");
    ...
    assert.equal(
      ...
      "Dispatch reads project files you open in the desktop app.",
    );
  });

  it("ad-hoc signs the complete development app bundle", () => {
    assert.deepEqual(resolveMacCodeSignArguments("/runtime/Dispatch (Dev).app"), [
      "--force",
      "--deep",
      "--sign",
      "-",
      "--timestamp=none",
      "/runtime/Dispatch (Dev).app",
    ]);
  });

  it("restores execute permissions on an unchanged launcher", () => {
    const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "dispatch-launcher-"));
  ...
  it("derives launcher icons from canonical development and production assets", () => {
    ...
    assert.match(development.sourceIconPath, /assets[\\/]dispatch[\\/]icon-1024\.png$/);
    assert.equal(development.generatedIconPath, "/runtime/icon-dev.icns");
    assert.match(production.sourceIconPath, /assets[\\/]dispatch[\\/]icon-1024\.png$/);
