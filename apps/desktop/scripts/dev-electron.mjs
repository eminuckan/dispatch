...
const remoteDebuggingPort = (
  process.env.DISPATCH_DESKTOP_REMOTE_DEBUGGING_PORT ??
  process.env.T3CODE_DESKTOP_REMOTE_DEBUGGING_PORT
)?.trim();
// oxlint-disable-next-line dispatch/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.DISPATCH_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.DISPATCH_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--dispatch-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--dispatch-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
