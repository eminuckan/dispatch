...
import { renderAssistantCitationsAsText } from "@dispatch/shared/assistantCitations";
...
import { resolveAssetUrl } from "@dispatch/client-runtime/state/assets";
...
} from "@dispatch/client-runtime/markdown-images";
import { resolveViewedImageAsset } from "@dispatch/client-runtime/work-log/presentation";
import {
  renderCodexFileCitationsAsMarkdown,
  splitCodexArtifactTemplateMarkdown,
} from "@dispatch/client-runtime/codex-markdown-directives";
import {
  CHAT_LIST_ANCHOR_OFFSET,
  resolveChatListAnchoredEndSpace,
} from "@dispatch/shared/chatList";
...
import { faviconUrlForOrigin } from "@dispatch/shared/favicon";
import {
...
import { markdownFileIconSource } from "@dispatch/mobile-markdown-text/file-icons";
import { PierreEntryIcon } from "../../components/PierreEntryIcon";
import { markdownLinkIconSource } from "@dispatch/mobile-markdown-text/link-icons";
import {
  normalizeNativeMarkdownUrl,
  resolveMarkdownInlineCodePresentation,
  resolveMarkdownLinkIcon,
  resolveMarkdownLinkPresentation,
} from "@dispatch/mobile-markdown-text/links";
import {
      ...
      >
        ...
        >
          ...
          <View className="min-w-0 flex-1 gap-1">
            <Text className="font-dispatch-medium text-sm text-foreground" numberOfLines={2}>
              {attachment.name}
            </Text>
...
function MarkdownInlineCode(props: {
  ...
  return (
    <NativeText
      className={presentation ? "font-dispatch-bold" : "font-mono"}
  ...
  return (
    <View className="my-2 min-w-0 flex-row items-center gap-3 rounded-2xl border border-border bg-card px-3 py-3">
      ...
      <View className="min-w-0 flex-1">
        <Text className="font-dispatch-bold text-sm text-foreground" numberOfLines={1}>
      ...
      {props.onUse ? (
        ...
        >
          <Text className="font-dispatch-bold text-xs text-foreground">Use template</Text>
        </Pressable>
      ) : null}
    </View>
  );
    ...
    return (
      ...
      >
        <Text
          ...
          className="font-dispatch-medium text-sm tabular-nums text-foreground-muted"
        >
          {entry.label}
        </Text>
        <ThreadDisclosureChevron
      ...
      >
        ...
        <View className="shrink-0 flex-row items-center gap-1.5">
          <SymbolView
            ...
            type="monochrome"
          />
          <Text className="font-dispatch-medium text-xs text-foreground-muted">{label}</Text>
        </View>
        <View className="h-px flex-1 bg-subtle" />
      </View>
    );
      ...
      return (
        <View className="mb-5 items-end">
          ...
          <View className="mt-1 flex-row items-center justify-end gap-1 pr-0.5">
            <Text className="font-dispatch-medium text-xs tabular-nums text-foreground-secondary">
  ...
  return (
    ...
    >
      <View className="max-w-[320px] items-center gap-2">
        <Text className="text-center font-dispatch-bold text-lg text-foreground">
          {props.title}
        </Text>
