...
describe("mobile theme runtime variables", () => {
  it("matches the standard base palette to the generated stylesheet", () => {
    expect(getMobileThemeRuntimeVariables("dispatch", "light", "web")).toEqual(
      readDefaultMobileThemeVariables("light"),
    );
    expect(getMobileThemeRuntimeVariables("dispatch", "dark", "web")).toEqual(
    ...
    (themeId) => {
      for (const appearance of ["light", "dark"] as const) {
        const base = getMobileThemeVariables(
          themeId === "material-you" ? "dispatch" : themeId,
          appearance,
        );
        ...
        expect(android).toEqual({
          ...
          "--color-header": themeColorWithAlpha(
            base[
              themeId === "dispatch" || themeId === "material-you"
                ? "--color-row-hover"
                : "--color-drawer"
            ],
            1,
  ...
  it.each(["dispatch", "material-you"] as const)(
    ...
    (themeId) => {
      for (const appearance of ["light", "dark"] as const) {
        ...
        if (appearance === "light") {
          ...
          expect(ios).toEqual(getMobileThemeVariables("dispatch", appearance));
