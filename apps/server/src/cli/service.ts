...
export function formatServiceStatus(
  ...
  if (!status.installed) {
    return "Dispatch service\n  Status: not installed\n  Next: Run `dispatch service install`.";
  }
  ...
  if (
    ...
    return [
      "Dispatch service",
      `  Status: installed · dispatch@${installedVersion} (newer than this dispatch@${cliVersion} CLI)`,
      `  Unit: ${status.unitPath}`,
      `  Logs: ${status.logPath}`,
      ...problems,
      `  Next: Run \`dispatch update ${installedVersion}\` to match it, or pass \`--allow-downgrade\` to \`dispatch service install\` explicitly.`,
    ].join("\n");
  }
  return [
    "Dispatch service",
    `  Status: ${status.current ? `installed · dispatch@${installedVersion}` : "needs an update or repair"}`,
    `  Unit: ${status.unitPath}`,
    `  Logs: ${status.logPath}`,
    ...problems,
    ...(status.current ? [] : ["  Next: Run `dispatch service install` to repair it."]),
  ].join("\n");
}
...
const serviceInstallCommand = Command.make("install", serviceReconcileFlags).pipe(
  Command.withDescription("Install Dispatch as a background service for this user."),
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!result.changed) {
          yield* Console.log(
            `Dispatch service is already installed with dispatch@${packageJson.version}.`,
          );
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} Dispatch service with dispatch@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
        );
      }),
    ),
  ),
...
const serviceUpdateCommand = Command.make("update", serviceReconcileFlags).pipe(
  Command.withDescription("Deprecated. Run `dispatch update` to move to a newer release."),
  Command.unlisted,
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        yield* Console.log(
          "`dispatch service update` is deprecated: run `dispatch update` to move to a newer release, or `dispatch service install` to repair the service. Repairing now.",
        );
        const result = yield* reconcileService({ allowDowngrade: flags.allowDowngrade });
        if (!result.changed) {
          yield* Console.log(`Dispatch service is already using dispatch@${packageJson.version}.`);
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} Dispatch service with dispatch@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
...
const serviceRestartCommand = Command.make("restart", projectLocationFlags).pipe(
  Command.withDescription(
    "Restart the background service. Picks up a version installed by `dispatch update` that was not restarted at the time.",
  ),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          restarted
            ? `Restarted the Dispatch service${status.installedVersion === undefined ? "" : ` on dispatch@${status.installedVersion}`}.`
            : "Dispatch service is not installed.",
        );
      }),
    ),
  ),
...
const serviceUninstallCommand = Command.make("uninstall", projectLocationFlags).pipe(
  Command.withDescription("Stop and remove the Dispatch background service."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          removed ? "Removed the Dispatch service." : "Dispatch service is not installed.",
        );
      }),
    ),
  ),
...
const serviceStatusCommand = Command.make("status", projectLocationFlags).pipe(
  Command.withDescription("Show whether the Dispatch background service is installed."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
...
export const offerServiceDuringOnboarding = Effect.gen(function* () {
  ...
  if (installed && current) {
    yield* Console.log("Dispatch is already set up to run in the background on this machine.");
    return true;
  }
  ...
  if (
    ...
    yield* Console.log(
      `A newer dispatch@${status.installedVersion} background service is installed. Leaving it unchanged.`,
    );
  ...
  const wanted = yield* Prompt.run(
    Prompt.Confirm({
      message: installed
        ? "The installed Dispatch service needs an update or repair. Update it now?"
        : platform === "darwin"
          ? "Run Dispatch in the background whenever you log in to this Mac? " +
            "It stays reachable through Dispatch Connect while you are logged in."
          : "Run Dispatch in the background whenever this machine boots? " +
            "It stays reachable through Dispatch Connect even after you log out.",
      initial: true,
    }),
  );
  if (!wanted) {
...
export const serviceCommand = Command.make("service").pipe(
  Command.withDescription("Manage the Dispatch background service."),
