...
/**
 * `dispatch theme` - inspect and set the environment's theme. Connected web and
 ...
 * Settings afterwards sticks until the next `dispatch theme set`.
 *
...
import { BUILT_IN_THEME_IDS, UNPUBLISHABLE_THEME_IDS } from "@dispatch/shared/themePalettes";
...
export class ThemeFileInvalidError extends Schema.TaggedError<ThemeFileInvalidError>()(
  ...
  override get message(): string {
    return `${this.filePath} is not a valid theme file. Use a theme exported from Dispatch, or a seeded file with name, appearance, canvas, and accent.`;
  }
}
...
export class ThemeTargetMissingError extends Schema.TaggedError<ThemeTargetMissingError>()(
  ...
  override get message(): string {
    return "Provide a theme id or file, or run `dispatch theme clear` to remove the theme.";
  }
}

const envDispatchHome = Config.String("DISPATCH_HOME").pipe(Config.option);
const envLegacyT3Home = Config.String("T3CODE_HOME").pipe(Config.option);

const resolveThemePaths = Effect.fn(function* (explicitBaseDir: Option.Option<string>) {
  // Same precedence as the rest of the CLI: --base-dir, then DISPATCH_HOME,
  // then legacy T3CODE_HOME, then the default home.
  const dispatchHome = Option.filter(yield* envDispatchHome, (value) => value.trim().length > 0);
  const legacyHome = Option.filter(yield* envLegacyT3Home, (value) => value.trim().length > 0);
  const envHome = Option.orElse(dispatchHome, () => legacyHome);
...
const themeSetCommand = Command.make("set", {
  ...
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      // make `dispatch theme set ocean` publish ./ocean whenever the cwd happens to
