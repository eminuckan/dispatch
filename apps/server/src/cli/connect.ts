...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? "Run `dispatch connect link` to authorize and enable T3 Connect."
    : !status.desired
      ? "Run `dispatch connect link` to enable T3 Connect."
      : !status.linked
    ...
    if (Exit.isFailure(input.relayResult)) {
      ...
      yield* Console.warn(
        input.clearAuthorization
          ...
          : "Could not revoke the relay-side environment record yet.\nRun `dispatch connect unlink` again when the relay is reachable.",
      );
    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!(yield* tokens.hasCredential)) {
          yield* Console.log(
            "Run `dispatch connect login` first so this environment can be authorized to publish.",
          );
          return;
        }
