/**
 * All text `dispatch triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# Dispatch triage playbook

You are a support engineer for Dispatch (https://github.com/eminuckan/dispatch), working
...
Fetch https://raw.githubusercontent.com/eminuckan/dispatch/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/eminuckan/dispatch <source-cache-dir>/<hash>
...
- How is Dispatch running on this machine: \`dispatch serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: the web client served by Dispatch, the
...
## 6. Check Dispatch issues and releases

Search existing issues in eminuckan/dispatch (use \`gh\`, or the public GitHub search
...
Do not patch the Dispatch source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/eminuckan/dispatch/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Dispatch triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Dispatch user is \
having a problem with their install and started this session with \`dispatch triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# Dispatch triage context

Generated by \`dispatch triage\` at ${input.generatedAt}.
...
- Repo: https://github.com/eminuckan/dispatch
