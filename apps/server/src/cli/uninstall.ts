...
/**
 * What `dispatch uninstall` would remove for one Dispatch home. Computed before anything
...
export const uninstallCommand = Command.make("uninstall", {
  ...
  Command.withDescription(
    "Remove Dispatch from this machine: the background service, the launcher, and every downloaded version. Your projects and threads are kept.",
  ),
...
const runUninstall = Effect.fn("cli.uninstall.run")(function* (input: {
  ...
  if (!plan.service && plan.launcher === undefined && plan.runtimeDir === undefined) {
    yield* Console.log(`Nothing to remove: Dispatch is not installed for ${input.baseDir}.`);
    if (!(yield* HostProcessIsExecutable)) {
      yield* Console.log(
        "  This Dispatch CLI runs from a Node script, so it was installed by npm or built from source. Remove it the same way (`npm uninstall -g t3`, or delete the checkout).",
      );
    }
    return;
  }
  ...
  if (!input.assumeYes) {
    ...
    const confirmed = yield* Prompt.run(
      Prompt.Confirm({ message: "Remove Dispatch from this machine?", initial: false }),
  ...
  if (plan.runtimeDir !== undefined) {
    ...
    if (platform === "win32") {
      ...
      yield* Console.log(`${runtimeDir} will be removed once Dispatch exits.`);
    } else {
      yield* fs
  ...
  yield* Console.log("");
  yield* Console.log("Dispatch is uninstalled. Thanks for trying Dispatch.");
});
