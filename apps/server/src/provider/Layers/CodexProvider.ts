...
import { PREFERRED_DEFAULT_CODEX_MODELS, ServerSettingsError } from "@dispatch/contracts";
...
export function buildCodexInitializeParams(): CodexSchema.V1InitializeParams {
  return {
    clientInfo: {
      name: "dispatch_desktop",
      title: "Dispatch Desktop",
    ...
    if (!codexSettings.enabled) {
      return buildServerProvider({
        ...
        probe: {
          ...
          status: "warning",
          auth: { status: "unknown" },
          message: "Codex is disabled in Dispatch settings.",
        },
      });
    }
