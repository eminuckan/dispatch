...
import { expandAssistantCitationsForProvider } from "@dispatch/shared/assistantCitations";
...
const makeProviderService = Effect.fn("makeProviderService")(function* (
    ...
    function* (threadId, rawInput) {
      ...
      return yield* Effect.gen(function* () {
        ...
        if (!instanceInfo.enabled) {
          return yield* toValidationError(
            "ProviderService.startSession",
            `Provider instance '${resolvedInstanceId}' is disabled in Dispatch settings.`,
          );
        }
  ...
  const sendTurn: ProviderServiceMethod<"sendTurn"> = Effect.fn("sendTurn")(function* (rawInput) {
    ...
    const flowInstructions = flowEnabled
      ? "<dispatch_flow>The user enabled Flow for this thread. You are the lead using the model they selected. Read flow_profiles for the configured worker pool: each profile describes its provider, model, effort, and suitable tasks. Prefer those descriptions when assigning work; use flow_models for the live catalog if no profile fits. Choose each worker deliberately; use routine workers for bounded tasks and deep workers when stronger reasoning is warranted. Workers may use other providers. Inspect the project before delegating: a Git project automatically gives workers isolated worktrees; a non-Git umbrella automatically gives them a shared workspace. For a task confined to one child Git repository, pass its path relative to the project root as repositoryPath to flow_spawn for an isolated worktree. Do not ask the user to initialize Git or choose the repo. Coordinate overlapping changes in shared workspaces. Give explicit assignments, read progress with flow_wait, message a working worker with flow_send, review every worker's result regardless of tier, integrate changes yourself, and give the final answer. Work directly when delegation would not help.</dispatch_flow>"
      : undefined;
