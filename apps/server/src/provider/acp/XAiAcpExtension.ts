...
import type { ProviderUserInputAnswers, UserInputQuestion } from "@dispatch/contracts";
...
/**
 * Client captured the plan for Dispatch's proposed-plan card. Abandon the native
 * Grok plan-approval gate so the turn unblocks; the user implements via Dispatch UI.
 */
 ...
 * Used so Dispatch can show the plan while plan mode is still active (before exit).
 */
...
export const makeXAiPromptCompletionRuntime = Effect.fn("makeXAiPromptCompletionRuntime")(
  function* (runtime: AcpSessionRuntime.AcpSessionRuntime["Service"]) {
    ...
    const allocatePromptFallbackId = Effect.sync(() => {
      ...
      return `dispatch-xai-prompt-${nextPromptFallbackId}`;
    });
