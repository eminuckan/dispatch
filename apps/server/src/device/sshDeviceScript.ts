...
const dispatchRoot = path.join(os.homedir(), '.dispatch', 'device');
const legacyRoot = path.join(os.homedir(), '.t3', 'device');
// Existing remote installs keep running in place. Fresh installs write only to Dispatch home.
const root = fs.existsSync(dispatchRoot) ? dispatchRoot : fs.existsSync(legacyRoot) ? legacyRoot : dispatchRoot;
