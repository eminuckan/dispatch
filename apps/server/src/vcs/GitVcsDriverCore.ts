...
import { dedupeRemoteBranchesWithLocalMatches, normalizeGitRemoteUrl } from "@dispatch/shared/git";
import { HostProcessPlatform } from "@dispatch/shared/hostProcess";
import { compactTraceAttributes } from "@dispatch/shared/observability";
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `dispatch-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
...
export const makeGitVcsDriverCore = Effect.fn("makeGitVcsDriverCore")(function* () {
  ...
  const prepareReviewIndex = Effect.fn("prepareReviewIndex")(function* (
    ...
    const tempIndexPath = yield* fileSystem.makeTempFileScoped({
      prefix: `dispatch-review-index-${process.pid}-`,
    });
    ...
    Effect.fn("refreshCheckedOutBranch")(function* (input) {
      ...
      if (!isAncestor) {
        ...
        yield* executeGit(
          ...
          input.cwd,
          ["update-ref", "refs/dispatch/pre-refresh", headCommit],
