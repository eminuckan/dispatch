...
import { ClaudeSettings, ProviderInstanceId } from "@dispatch/contracts";
import { HostProcessPlatform, isHostWindows } from "@dispatch/shared/hostProcess";
...
const ClaudeTextGenerationTestLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
  prefix: "dispatch-claude-text-generation-test-",
...
function withFakeClaudeEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    ...
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "dispatch-claude-text-" });
