...
export const make = Effect.gen(function* () {
  ...
  const verify = Effect.fn("TeamRuntime.verify")(function* (
    cwd: string,
    checks: ReadonlyArray<{ command: string; args: ReadonlyArray<string> }>,
  ) {
    return yield* Effect.forEach(checks, (check) =>
      runProcess({ cwd, command: check.command, args: check.args }).pipe(
        Effect.map((result) => ({
  ...
  const reviewWorker = Effect.fn("TeamRuntime.reviewWorker")(function* (
    ...
    const work = run.attempts.findLast(
      (attempt) =>
        attempt.taskId === task.id && attempt.role === "work" && attempt.status === "succeeded",
    );
    if (!work) return run;
    ...
    if (evidence.some((entry) => !entry.passed)) {
      const correction = encodeJson({ summary: review.summary, evidence });
      if (attemptCount(run, task.id, "work") >= run.policy.maxAttempts)
        return yield* store.update(
          run.id,
          run.revision,
    ...
    return yield* store.update(
      ...
      (current) => ({
        ...
        decisions: [
          ...current.decisions,
          encodeJson({ task: task.id, review: review.summary, evidence }),
        ],
        tasks: current.tasks.map((candidate) =>
          candidate.id === task.id
  ...
  const advanceIntegration = Effect.fn("TeamRuntime.advanceIntegration")(function* (run: TeamRun) {
    ...
    if (evidence.some((entry) => !entry.passed)) {
      ...
      return yield* appendAttempt(
        ...
        makeAttempt({
          ...
          role: "integrate",
          taskId: null,
          prompt: integrationCorrectionPrompt(
            run,
            encodeJson({ summary: result.summary, evidence }),
          ),
          createdAt,
        }),
    ...
    const completed = yield* store.update(
      ...
      (current) => ({
        ...
        decisions: [...current.decisions, JSON.stringify({ summary: result.summary, evidence })],
      }),
      "run-completed",
    );
        ...
        status:
          input.action === "cancel"
            ...
            : input.action === "pause"
              ? "paused"
              : current.attempts.some(
                    (attempt) => attempt.role === "plan" && attempt.status !== "succeeded",
                  )
                ? "planning"
  ...
  const providerDecision = Effect.fn("TeamRuntime.providerDecision")(function* (
    ...
    return yield* applyFailover(run, failover, input.action, input.profileId, "user");
  });
