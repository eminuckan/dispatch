  ...
  it.effect("runs the inherited machine setup action in the checkout's worktree", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(open).toHaveBeenCalledWith({
        ...
        worktreePath: "/repo/worktrees/a",
        env: {
          DISPATCH_PROJECT_ROOT: "/repo/project",
          DISPATCH_WORKTREE_PATH: "/repo/worktrees/a",
  ...
  it.effect(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const sentinel = /__DISPATCH_SETUP_DONE___[0-9a-f]{32}:/.exec(written)?.[0];
        ...
        yield* emit("__DISPATCH_SETUP_DONE__:0\r\n");
        yield* emit(`__DISPATCH_SETUP_DONE___${"0".repeat(32)}:0\r\n`);
        ...
        expect(seen).toEqual([
          ...
          "Progress: 3/3",
          "Done in 2s",
          "__DISPATCH_SETUP_DONE__:0",
          `__DISPATCH_SETUP_DONE___${"0".repeat(32)}:0`,
        ]);
  ...
  it.effect.each([
    {
      shell: "/usr/bin/fish",
      expected:
        /^begin\rbun install\rend; printf '\\n__DISPATCH_SETUP_DONE___[0-9a-f]{32}:%s\\n' \$status\r$/,
    },
    {
      shell: "/bin/bash",
      expected:
