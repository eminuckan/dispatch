...
it.effect("uses one EnvironmentAuth pairing grant for both QR and short-code presentation", () =>
  Effect.gen(function* () {
    ...
    const challenge = yield* issueDispatchConnectPairingChallenge({
      environmentAuth,
      dispatchConnect,
...
it.effect(
  "revokes the local pairing grant when configured Connect rendezvous registration fails",
  () =>
    Effect.gen(function* () {
