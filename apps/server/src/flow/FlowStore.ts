...
export const make = Effect.gen(function* () {
  ...
  const getWorkerRow = (threadId: string) =>
    sql<WorkerRow>`SELECT thread_id AS "threadId", parent_thread_id AS "parentThreadId", spawn_id AS "spawnId", assignment, model_selection_json AS "modelSelectionJson", branch, worktree_path AS "worktreePath", state, error, created_at AS "createdAt", updated_at AS "updatedAt" FROM flow_workers WHERE thread_id = ${threadId}`.pipe(
      Effect.map((rows) => rows[0] ?? null),
    );
  const getJobRow = (id: string) =>
    sql<JobRow>`SELECT id, worker_thread_id AS "workerThreadId", command_id AS "commandId", message_id AS "messageId", prompt, state, result, error, created_at AS "createdAt", updated_at AS "updatedAt" FROM flow_jobs WHERE id = ${id}`.pipe(
      Effect.map((rows) => rows[0] ?? null),
    );
  const latestJobRow = (workerThreadId: string) =>
    sql<JobRow>`SELECT id, worker_thread_id AS "workerThreadId", command_id AS "commandId", message_id AS "messageId", prompt, state, result, error, created_at AS "createdAt", updated_at AS "updatedAt" FROM flow_jobs WHERE worker_thread_id = ${workerThreadId} ORDER BY created_at DESC, rowid DESC LIMIT 1`.pipe(
      Effect.map((rows) => rows[0] ?? null),
    );
  ...
  const getWorkerBySpawnId = Effect.fn("FlowStore.getWorkerBySpawnId")(function* (spawnId: string) {
    const rows =
      yield* sql<WorkerRow>`SELECT thread_id AS "threadId", parent_thread_id AS "parentThreadId", spawn_id AS "spawnId", assignment, model_selection_json AS "modelSelectionJson", branch, worktree_path AS "worktreePath", state, error, created_at AS "createdAt", updated_at AS "updatedAt" FROM flow_workers WHERE spawn_id = ${spawnId}`;
  ...
  const listWorkers = Effect.fn("FlowStore.listWorkers")(function* (parentThreadId: ThreadId) {
    const rows = yield* sql<WorkerRow>`
      SELECT thread_id AS "threadId", parent_thread_id AS "parentThreadId", spawn_id AS "spawnId", assignment, model_selection_json AS "modelSelectionJson", branch, worktree_path AS "worktreePath", state, error, created_at AS "createdAt", updated_at AS "updatedAt" FROM flow_workers WHERE parent_thread_id = ${parentThreadId}
  ...
  const reserveWorker = Effect.fn("FlowStore.reserveWorker")(function* (input: {
    ...
    return yield* sql
      .withTransaction(
        Effect.gen(function* () {
          const prior =
            yield* sql<WorkerRow>`SELECT thread_id AS "threadId", parent_thread_id AS "parentThreadId", spawn_id AS "spawnId", assignment, model_selection_json AS "modelSelectionJson", branch, worktree_path AS "worktreePath", state, error, created_at AS "createdAt", updated_at AS "updatedAt" FROM flow_workers WHERE spawn_id = ${input.spawnId}`;
          if (prior[0]) {
            if (
          ...
          SELECT COUNT(*) AS count FROM flow_workers
          WHERE parent_thread_id = ${input.parentThreadId} AND state != 'stopped'
        `;
          if ((count[0]?.count ?? 0) >= 5)
            return yield* fail(
              "conflict",
              "This Flow already has five workers. Stop one before adding another.",
            );
          yield* sql`
          INSERT INTO flow_workers (thread_id, parent_thread_id, spawn_id, assignment, model_selection_json, branch, worktree_path, state, error, created_at, updated_at)
          VALUES (${input.threadId}, ${input.parentThreadId}, ${input.spawnId}, ${input.assignment}, ${encodeSelection(input.modelSelection)}, ${input.branch}, NULL, 'queued', NULL, ${input.createdAt}, ${input.createdAt})
        `;
          yield* sql`
  ...
  const reserveJob = Effect.fn("FlowStore.reserveJob")(function* (input: {
    ...
    return yield* sql
      .withTransaction(
        Effect.gen(function* () {
          const prior =
            yield* sql<JobRow>`SELECT id, worker_thread_id AS "workerThreadId", command_id AS "commandId", message_id AS "messageId", prompt, state, result, error, created_at AS "createdAt", updated_at AS "updatedAt" FROM flow_jobs WHERE id = ${input.id}`;
          if (prior[0]) {
            if (
          ...
          if (worker.state !== "idle")
            return yield* fail(
              "conflict",
              "Worker is not idle. Wait for its current task or stop it.",
            );
          yield* sql`
          INSERT INTO flow_jobs (id, worker_thread_id, command_id, message_id, prompt, state, result, error, created_at, updated_at)
          VALUES (${input.id}, ${input.workerThreadId}, ${input.id}, ${input.messageId}, ${input.message}, 'queued', NULL, NULL, ${input.createdAt}, ${input.createdAt})
        `;
  ...
  const activeJobs = Effect.gen(function* () {
    const rows =
      yield* sql<JobRow>`SELECT id, worker_thread_id AS "workerThreadId", command_id AS "commandId", message_id AS "messageId", prompt, state, result, error, created_at AS "createdAt", updated_at AS "updatedAt" FROM flow_jobs WHERE state IN ('queued', 'working') ORDER BY created_at ASC, rowid ASC`;
    return rows;
  ...
  const updateJob = Effect.fn("FlowStore.updateJob")(function* (input: {
    ...
    yield* sql
      ...
      `;
          yield* sql`
        UPDATE flow_workers SET state = ${input.workerState}, error = ${input.error ?? null}, updated_at = ${input.updatedAt}
