...
Load this reference only when inspecting or seeding local Dispatch state directly.
...
```bash
node apps/server/scripts/dispatch-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/dispatch-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/dispatch-seed.sql
```
