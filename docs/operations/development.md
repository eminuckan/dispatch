...
Flags go directly after the task name, for example `vp run dev --home-dir /tmp/dispatch-dev`.
Add `--browser` to open a browser automatically.

### State and ports

Fresh linked worktrees default to their own `.dispatch/userdata`. If a worktree already has legacy
`.t3` state and no `.dispatch`, Dispatch adopts that directory in place; it does not copy or delete
it. If both exist, `.dispatch` wins.

An explicit `--home-dir` has highest precedence. `DISPATCH_HOME` is the canonical environment
override and wins over the worktree default; `T3CODE_HOME` remains a lower-priority compatibility
fallback. In the main checkout, the server uses `~/.dispatch/dev/userdata` for fresh state and adopts
`~/.t3/dev/userdata` when only the legacy home exists.

Never point a development server at the main installation's `userdata` directory.
...
but occupied ports can shift them. `DISPATCH_PORT_OFFSET` or `DISPATCH_DEV_INSTANCE` can select a
...
`DISPATCH_BUNDLED_DEV=0` opts out when debugging bundler differences. Two reload traps matter
...
including services unrelated to Dispatch. If you run untrusted services on that hostname, keep
