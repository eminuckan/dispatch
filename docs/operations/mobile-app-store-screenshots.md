...
non-rendering pairing/readiness coordinator and disables terminal autofocus so captures do not contain
the software keyboard. Environment cards come from real paired servers.
...
Every configured device defaults to dark appearance and the `dispatch` palette, so plain
...
Pass `--theme <id>` (repeatable) or `--theme all` to capture the app's other palettes: `dispatch`,
`dispatch-chat`, `grove`, `ocean`, `ember`, and `iris`. The runner hands the palette to the app as a launch
...
| -------------------------------------- | ------------------------- | ----------------- | ----------------------------------------- |
| `apple/iphone-6.9/dark/dispatch/`      | iPhone 17 Pro Max         | 1320×2868         | App Store Connect iPhone 6.9-inch         |
| `apple/iphone-6.5/dark/dispatch/`      | disposable iPhone 14 Plus | 1284×2778         | App Store Connect iPhone 6.5-inch         |
| `apple/ipad-13/dark/dispatch/`         | iPad Pro 13-inch (M5)     | 2752×2064         | App Store Connect iPad 13-inch, landscape |
| `google-play/phone/dark/dispatch/`     | Pixel AVD at 420 dpi      | 1080×1920         | Google Play phone, portrait 9:16          |
| `google-play/tablet-7/dark/dispatch/`  | Pixel AVD at 600dp width  | 1080×1920         | Google Play 7-inch tablet, portrait 9:16  |
| `google-play/tablet-10/dark/dispatch/` | Pixel AVD at 800dp width  | 1440×2560         | Google Play 10-inch tablet, portrait 9:16 |
...
The generated tree is deliberately aligned with the store upload fields:
    ...
    ├── apple/
    │   ├── iphone-6.9/dark/dispatch/{thread,terminal,review,threads,environments}.png
    │   ├── iphone-6.5/dark/dispatch/{thread,terminal,review,threads,environments}.png
    │   └── ipad-13/dark/dispatch/{thread,terminal,review,threads,environments}.png
    └── google-play/
        ├── phone/dark/dispatch/{thread,terminal,review,threads,environments}.png
        ├── tablet-7/dark/dispatch/{thread,terminal,review,threads,environments}.png
        └── tablet-10/dark/dispatch/{thread,terminal,review,threads,environments}.png

A light-only run writes the same tree under `light/`; `--appearance both` writes both appearance
folders, and each requested theme adds a sibling folder next to `dispatch/`.
...
timeout from 60 to 300 minutes). The default workflow run captures both appearances of the `dispatch`
...
`pnpm --filter @dispatch/mobile showcase` starts Expo on its normal port, so it is not compatible with
...
The Pending rows use the production offline outbox and point at the real Dispatch and React fixture
