...
`agent-device` command line, which Dispatch sets up for them.
...
Studio's Device Manager. Dispatch detects standard SDK locations; set
...
Agents drive the device through the `agent-device` command line. Dispatch
...
local network, Tailscale, and Dispatch Connect. Live video needs a secure page
...
Dispatch checks common Homebrew and Android SDK locations; custom installations need
...
sessions and stops its Dispatch helpers when reachable; simulators keep running.

Dispatch provides discovery, streaming, and control. Arrange app builds,
