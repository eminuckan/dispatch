...
[Gitea CLI (`tea`)](https://gitea.com/gitea/tea) 0.16 or later on your Dispatch server.
Sign in with `fj --host https://your-server auth add-token` or `tea login add`.
Repeat for each server you use, including Codeberg.

Dispatch prefers a matching `fj` login and falls back to `tea` when `fj` is unavailable
...
Use a thread's Git actions to commit, push, and create a pull request. Dispatch can generate commit
...
On GitHub these are GitHub's own viewed marks, so a review carries between Dispatch and github.com
in either direction. Forgejo, GitLab, Bitbucket, and Azure DevOps expose no record Dispatch can read, so the
server you are connected to keeps them instead: they follow you across the apps connected to that
server, but the host's own site will not show them, and the count reads **viewed in Dispatch**.
