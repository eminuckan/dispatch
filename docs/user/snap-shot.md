...
Shift keys together. Dispatch attaches the image to your draft and brings itself forward. If no thread
is open it starts a draft in the current project.

Pressing the shortcut while Dispatch is in front captures Dispatch itself.
...
a modifier pair such as Command+Command or Ctrl+Ctrl, or a key chord. Dispatch refuses shortcuts
...
terminal contents. If an app is slow to answer, Dispatch attaches the screenshot without the data
rather than waiting.
...
**GNOME.** Install the bundled **Dispatch SnapShots** extension during setup. It is installed
...
shortcut does not fire, check Dispatch under **System Settings** > **Keyboard** > **Shortcuts**. If a
...
**Review changes**. Dispatch shows the exact change it will make to your Hyprland config. **Save
...
**Niri.** Choose a shortcut and select **Review changes**. Dispatch shows the exact binding it will
...
**Other Wayland desktops.** Dispatch uses the desktop's screenshot portal when it can capture the
