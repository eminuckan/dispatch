...
Dispatch uses Claude Code's login and configuration. Start with the default provider
...
Dispatch uses the Claude configuration on the connected server.
...
Select the model you want in Dispatch. For an OpenRouter model outside the built-in
...
do not replace the explicit model ID selected in Dispatch. Custom models may have
