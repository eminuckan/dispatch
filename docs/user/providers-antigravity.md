...
Dispatch runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for Dispatch to confirm
...
hostname. Only that Dispatch sign-in session can finish the attempt. If it expires,
...
Use Antigravity's native `/plan` command for planning. Dispatch's separate Plan mode
...
Dispatch keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. Dispatch also reads `.gemini/skills` and the
