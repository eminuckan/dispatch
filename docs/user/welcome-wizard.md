...
Dispatch shows a setup flow when you open a new installation or connect with a
browser client for the first time. Existing workspaces skip this flow.

## Connect your computers

Select one or more computers to set up. If you opened Dispatch directly from a
...
- **Add a computer** connects directly to a server on your network or tailnet.
  Follow [direct pairing](./remote-access.md#pair-over-a-lan-or-private-network) to start a reachable
  server and create a pairing link. The guide covers both the compatibility launcher and source build.

Saved computers are selected by default. Uncheck any you do not want to set up; this does not
disconnect them.
...
If Dispatch cannot confirm the workspace during startup, the setup flow shows
**Still connecting** instead of opening the app. Select **Reload** to try again.

If Dispatch cannot read your saved settings, it shows **Could not read settings**.
Select **Retry** after storage becomes available. Setup does not replace
unreadable settings with defaults.

## Check your agents

Dispatch checks each selected computer for Claude Code and Codex. If an agent is
...
Dispatch finds directories that Claude Code or Codex has used. Git repositories
...
A large or malformed history can reach the scan limit. Dispatch keeps the
...
30 days. You can continue those conversations in Dispatch.

Conversation import is best effort. Dispatch keeps the first user prompt and the
...
conversation so Dispatch does not remove user text. It reads one conversation at
