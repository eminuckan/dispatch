...
- The full dev stack: `vp run dev`, then open the pairing URL it prints through the forwarded web port (5733). The bare origin is useless without the pairing token. In VS Code the forwarded port is a true localhost, so the printed URL works as-is; in browser Codespaces the forwarded origin differs, and if the server rejects it, pass the forwarded origin via `DISPATCH_DEV_ALLOWED_ORIGINS`.
...
`DISPATCH_HOME` points at the workspace's gitignored `.dispatch`, so all runtime state stays inside the container workspace, mirroring the fresh worktree default. There is no live install to damage inside a container, but the test-data rule from AGENTS.md still holds: copy data in, never point at shared state. `T3CODE_HOME` and an existing `.t3` remain legacy compatibility inputs outside this fresh container setup.
...
```bash
devcontainer build --workspace-folder . --push true --image-name <registry>/dispatch-devcontainer:latest
```
