...
npm-installed at a pinned version into the Dispatch home after its matching Device
...
session arbitrary command execution on the host, so Dispatch does not. The
...
snapshot model agents need and stays current with its own releases. Dispatch prepends
