...
const packageRepo = {
  ...
  packageVersionPath: ["dependencies", "example"],
  versionTagPrefix: "v",
};
...
it.layer(NodeServices.layer)("sync-reference-repos", (it) => {
  ...
  it.effect("preserves version source parse context and the schema cause", () =>
    Effect.gen(function* () {
      ...
      const sourcePath = path.join(rootDir, packageRepo.versionSourcePath);
      yield* fs.makeDirectory(path.dirname(sourcePath), { recursive: true });
      yield* fs.writeFileString(sourcePath, "{");

      const error = yield* resolveReferenceRepoRef(packageRepo, rootDir, false).pipe(Effect.flip);

      if (error._tag !== "ReferenceRepoVersionSourceError") {
        assert.fail(`Unexpected error: ${error._tag}`);
      }
      assert.equal(error.operation, "parse");
      assert.equal(error.repoId, packageRepo.id);
      assert.equal(error.sourcePath, sourcePath);
      assert.ok(error.cause !== undefined);
      assert.ok(!error.message.includes(String((error.cause as Error).message)));
    }),
  ...
  it.effect("reports the unresolved package path without inventing a cause", () =>
    Effect.gen(function* () {
      ...
      assert.equal(error.repoId, packageRepo.id);
      assert.equal(error.sourcePath, sourcePath);
      assert.deepStrictEqual(error.packageVersionPath, ["dependencies", "example"]);
      assert.ok(!("cause" in error));
    }),
  );

  it.effect("resolves a reference tag from a package dependency", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const rootDir = yield* fs.makeTempDirectoryScoped({
        prefix: "sync-reference-repos-package-version-",
      });
      yield* fs.writeFileString(
      ...
      assert.equal(yield* resolveReferenceRepoRef(packageRepo, rootDir, false), "v2.0.0-beta.49");
    }),
  );
