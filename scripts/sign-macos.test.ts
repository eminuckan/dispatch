...
it("batches codesign calls without changing existing signing options", async () => {
  const options = {
    app: "/tmp/Dispatch.app",
    identity: "Developer ID Application: Dispatch Test",
    keychain: "/tmp/dispatch.keychain",
    provisioningProfile: "/tmp/dispatch.provisionprofile",
    optionsForFile: () => ({
      entitlements: "/tmp/dispatch.entitlements.plist",
