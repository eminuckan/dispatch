...
test("loopback origin validation normalizes supported local hosts", () => {
  assert.equal(normalizeLoopbackOrigin("http://localhost:8787"), "http://127.0.0.1:8787");
  assert.equal(normalizeLoopbackOrigin("http://127.0.0.1:3000/"), "http://127.0.0.1:3000");
  assert.throws(() => normalizeLoopbackOrigin("https://127.0.0.1:8787"), ManagedTunnelError);
  assert.throws(() => normalizeLoopbackOrigin("http://192.168.1.10:8787"), ManagedTunnelError);
  assert.throws(() => normalizeLoopbackOrigin("http://127.0.0.1:8787/path"), ManagedTunnelError);
});

test("managed tunnel identity is stable and constrained to the configured base domain", () => {
  assert.deepEqual(
    managedTunnelIdentity({ id: "env-1234-ABCDE", label: "Emin's Mac mini" }, "remote.example.com"),
    {
...
test("Cloudflare client emits the documented tunnel, ingress, DNS, and token requests", async () => {
  ...
  assert.equal(await client.getTunnelToken(tunnel.id), "connector-token");
  assert.equal(dns.id, "dns-1");
...
test("managed tunnel ensure reuses allocation and never persists connector tokens", async () => {
  ...
  const fetcher = async (input: string | URL | Request, init: RequestInit = {}) => {
    ...
    if (url.endsWith("/token")) return Response.json({ success: true, result: "connector-token" });
    return Response.json({ success: true, result: {} });
  };
  ...
  assert.equal(first.connectorToken, "connector-token");
  assert.equal(second.connectorToken, "connector-token");
  assert.equal(createCount, 1);
  assert.equal(dnsCount, 1);
  assert.equal(persisted.length, 1);
  assert.equal(JSON.stringify(persisted).includes("connector-token"), false);
});

test("managed tunnel delete cleans local state after partial remote cleanup", async () => {
  ...
  assert.deepEqual(result, { remoteCleanup: "partial" });
  assert.equal(localDeleted, true);
  assert.equal(deleted.length, 2);
});

test("Cloudflare API failures become stable upstream errors without response leakage", async () => {
