...
describe("PublishClientConfig", () => {
  test.provider("writes the env file on deploy and skips an unchanged redeploy", (stack) =>
    Effect.gen(function* () {
      ...
      const dir = yield* fs.makeTempDirectoryScoped({ prefix: "dispatch-relay-client-config-" });
      const target = path.join(dir, "client.env");
      yield* fs.writeFileString(target, "KEEP=yes\n");
      const configured = Effect.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({ DISPATCH_RELAY_CLIENT_CONFIG_ENV: target }),
        ),
      );
      ...
      expect(first).toContain("DISPATCH_RELAY_URL=https://relay.example.com\n");
      expect(first).toContain("DISPATCH_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-v1\n");
      expect(first).toContain("DISPATCH_MOBILE_OTLP_TRACES_TOKEN=mobile-v1\n");
      ...
      expect(third).toContain("DISPATCH_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-v2\n");
